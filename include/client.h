 ...
 * @version $Id: client.h,v 1.6 2005/12/25 17:26:59 progs Exp $
 */
...
enum Priv
  {
    PRIV_JOINX, /**< oper can use /join x to override blocking join modes (+blki) */
    PRIV_CHAN_LIMIT, /**< no channel limit on oper */
    PRIV_MODE_LCHAN, /**< oper can mode local chans */
    PRIV_PROTECT, /**< oper can use +p mode, which prevents him from being deopped or kicked, enables him to bypass socket penalties and +bmcCN chanmode  */
    ...
    PRIV_OPERFREEMODE, /**< can /mode even if not opped */
...
enum Flag
  {
    ...
    FLAG_PROTECTED,                 /**< oper is protected */
    FLAG_SETHOST,                   /**< user's got a virtual host */
...
/** Return non-zero if the client is protected with +p */
#define IsProtected(x)          HasFlag(x, FLAG_PROTECTED)
/** Return non-zero if the client has a virtual host */
...
/** Mark a client as having mode +p (protected) */
#define SetProtected(x)         SetFlag(x, FLAG_PROTECTED)
/** Mark a client as having mode +H (virtual host) */
...
/** Remove mode +p (protected) from the client. */
#define ClearProtected(x)       ClrFlag(x, FLAG_PROTECTED)
/** Remove mode +H (virtual host) from the client. */
...
#define SNO_OPERDEFAULT (SNO_DEFAULT|SNO_HACK2|SNO_HACK4|SNO_THROTTLE|SNO_OLDSNO|SNO_OLDREALOP)
