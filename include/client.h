 ...
 *
 *
 * $Id: client.h,v 1.21 2006/02/23 05:55:13 bugs Exp $
 */
...
#define OFLAG_REHASH     0x020
#define OFLAG_DIE        0x040
#define OFLAG_GLINE      0x080
#define OFLAG_SETVARS    0x1000
#define OFLAG_SA         0x2000
#define OFLAG_INVISIBLE	 0x4000

#define OSetGlobal(x)		(cli_oflags(x) |= OFLAG_GLOBAL)
#define OSetAdmin(x)		(cli_oflags(x) |= OFLAG_ADMIN)
#define OSetRehash(x)		(cli_oflags(x) |= OFLAG_REHASH)
#define OSetDie(x)		(cli_oflags(x) |= OFLAG_DIE)
#define OSetSetVars(x)		(cli_oflags(x) |= OFLAG_SETVARS)
#define OSetGline(x)		(cli_oflags(x) |= OFLAG_GLINE)
#define OSetSA(x)               (cli_oflags(x) |= OFLAG_SA)
#define OSetInv(x)		(cli_oflags(x) |= OFLAG_INVISIBLE)

#define OpIsGlobal(x)		(cli_oflags(x) & OFLAG_GLOBAL)
#define OpIsAdmin(x)		(cli_oflags(x) & OFLAG_ADMIN)
#define CanRehash(x)		(cli_oflags(x) & OFLAG_REHASH)
#define CanDie(x)		(cli_oflags(x) & OFLAG_DIE)
#define CanSetVars(x)		(cli_oflags(x) & OFLAG_SETVARS)
#define CanGline(x)		(cli_oflags(x) & OFLAG_GLINE)
#define CanSA(x)                (cli_oflags(x) & OFLAG_SA)
#define CanInv(x)		(cli_oflags(x) & OFLAG_INVISIBLE)

#define OClearGlobal(x)		(cli_oflags(x) &= ~OFLAG_GLOBAL)
#define OClearAdmin(x)		(cli_oflags(x) &= ~OFLAG_ADMIN)
#define OClearRehash(x)		(cli_oflags(x) &= ~OFLAG_REHASH)
#define OClearDie(x)		(cli_oflags(x) &= ~OFLAG_DIE)
#define OClearSetVars(x)	(cli_oflags(x) &= ~OFLAG_SETVARS)
#define OClearGline(x)		(cli_oflags(x) &= ~OFLAG_GLINE)
#define OClearSA(x)             (cli_oflags(x) &= ~OFLAG_SA)
#define OClearInv(x)		(cli_oflags(x) &= ~OFLAG_INVISIBLE)
...
enum Flag {
    ...
    FLAG_MALE,                      /* pour nous, les +hommes */
    FLAG_FEMALE,                    /* +f */
    FLAG_ADMIN,                     /* admins */
    FLAG_WANTRESTART,               /* +D, pour restart/die */
    FLAG_PROTECT,                   /* protect me, my god */
    FLAG_SETHOST,                   /* host slinée */
    FLAG_NOPRIVATE,                 /* pas de messages privés */
    FLAG_HELPER,                    /* flag des helpers */
    FLAG_PACCONLY,                  /* reçoi les messages uniquement des users logués */
    FLAG_NOIDLE,		    /* n'affiche pas l'idle */
    FLAG_NOCHAN,		    /* n'affiche pas les chanels */
    FLAG_WHOIS,			    /* affiche si on se fait whoisifié */
    FLAG_VHOST,			    /* Vhost */
    FLAG_SSL,                       /* SSL user */
    FLAG_HIDE,			    /* invisilité total */
    FLAG_HIDEOPER,		    /* pas d'info dans /whois /who ircops */
...
#define IsAnOper(x)             HasFlag(x, FLAG_OPER)
