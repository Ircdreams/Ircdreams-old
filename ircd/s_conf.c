 ...
 *
 * $Id: s_conf.c,v 1.29 2006/02/23 05:55:13 bugs Exp $
 */
#include "../config.h"
...
static int oper_access[] = {
	OFLAG_GLOBAL,	 'O',	// permet de s'op partout
	OFLAG_ADMIN,	 'A',	// contient +ORDG
	OFLAG_REHASH,	 'R',	// rehash the server
	OFLAG_DIE,	 'D',	// die the server
	OFLAG_SETVARS,	 'V',	// use /set
	OFLAG_GLINE,	 'G',	// use /gline
	OFLAG_SA,        'S',   // Permet d'utiliser les commandes SVS
	OFLAG_INVISIBLE, 'X',	// Permer d'utilisation du mode +X
	0, 0
};
...
{
	int *i, flag;
	static char oflagbuf[sizeof oper_access / sizeof *oper_access];
	char *p = oflagbuf;
...
static void killcomment(struct Client* sptr, const char* filename)
{
  ...
  if (NULL == (file = fbopen(filename, "r"))) {
    ...
    send_reply(sptr, SND_EXPLICIT | ERR_YOUREBANNEDCREEP,
               ":La connexion depuis votre host est refusé par ce serveur.");
    return;
  }
  fbstat(&sb, file);
...
void conf_add_mapping(const char* const* fields, int count)
{
   struct s_map *map;

   if (count < 3 || EmptyString(fields[1]) || EmptyString(fields[2]) || EmptyString(fields[3]))
     return;

   map = (struct s_map *)MyMalloc(sizeof(struct s_map));

   DupString(map->command, fields[1]);
   DupString(map->nick, fields[2]);
   DupString(map->serveur, fields[3]);
   if (!EmptyString(fields[4])) DupString(map->prepend, fields[4]);
   map->flags |= MFLG_SLOW;
...
int read_configuration_file(void)
{
  ...
  while (fbgets(line, sizeof(line) - 1, file)) {
    ...
    if (':' != line[1]) {
      Debug((DEBUG_ERROR, "Mauvaise configuration de la ligne: %s", line));
      sendto_opmask_butone(0, SNO_OLDSNO,"Mauvaise Configuration de ligne");
      continue;
    }
    ...
    *dest = '\0';
    ...
    case 'B':
    case 'b':
      conf_add_mapping(field_vector, field_count);
      aconf->status = CONF_ILLEGAL;
      break;
    ...
    default:
      ...
      sendto_opmask_butone(0, SNO_OLDSNO, "Prefixe inconnu dans le fichier de configuration: %c",
			   *field_vector[0]);
      aconf->status = CONF_ILLEGAL;
      break;
    }
    ...
    {
      if (aconf->status & CONF_OPERATOR)    /* oflags */
	  {
		int* i, flag;
		const char *m = *field_vector[4] ? field_vector[4] : "O";
		/*
		 * Now we use access flags to define
		 * what an operator can do with their O line.
		 */
		for (; *m; m++)
		{
			for (i = oper_access; (flag = *i); i += 2)
...
int rehash(struct Client *cptr, int sig)
{
  ...
  if (1 == sig)
    sendto_opmask_butone(0, SNO_OLDSNO,
                         "Reçu un signal SIGHUP, rechargement du fichier ircd.conf");
    ...
    if ((acptr = LocalClientArray[i])) {
      ...
      if ((found_g = find_kill(acptr))) {
        sendto_opmask_butone(0, found_g == -2 ? SNO_GLINE : SNO_OPERKILL,
                             found_g == -2 ? "G-line actif pour %s%s" :
                             "K-line actif pour %s%s",
                             IsUnknown(acptr) ? "un Client non enregistré ":"",
...
int find_kill(struct Client *cptr)
{
  ...
  if (deny) {
    if (EmptyString(deny->message))
      send_reply(cptr, SND_EXPLICIT | ERR_YOUREBANNEDCREEP,
                 ":Les connexions depuis cette host sont refusées sur ce serveur.");
    else {
  ...
  if (deny || agline)
    send_reply(cptr, SND_EXPLICIT | ERR_YOUREBANNEDCREEP,
                 ":Pour plus d'informations, envoyez un mail à %s.", feature_str(FEAT_KLINE_MAIL));
...
int conf_check_server(struct Client *cptr)
{
  ...
  if (IsConnecting(cptr) || IsHandshake(cptr)) {
    ...
    if (!c_conf) {
      sendto_opmask_butone(0, SNO_OLDSNO, "Erreur de Connexion: C:line perdue pour %s",
                           cli_name(cptr));
...
void conf_add_sline(const char* const* fields, int count)
{
  ...
  if (count < 2 || EmptyString(fields[1]) || EmptyString(fields[2]) || EmptyString(fields[3]))
    return;
  ...
  if (!EmptyString(fields[2])) {
    DupString(sline->realhost, fields[2]);
  ...
  if (!EmptyString(fields[3]))
    DupString(sline->username, fields[3]);
  else
    sline->username = NULL;
  ...
  for (sconf = GlobalSList; sconf; sconf = sconf->next) {
    ...
    if (match(sconf->username, cli_user(cptr)->username) == 0) {
     ...
     if ((hostonly = strchr(sconf->spoofhost, '@')))
      ...
      ircd_strncpy(cli_user(cptr)->crypt, hostonly, HOSTLEN);
