 ...
 * @version $Id: s_conf.c,v 1.4 2006/03/26 18:04:02 progs Exp $
 */
#include "config.h"
...
struct DenyConf*   denyConfList = 0;
/** Global list of S-lines */
struct VHostConf*  vhostList = 0;
...
/** Free all vhosts from #vhostList. */
void conf_erase_vhost_list(void)
{
  struct VHostConf* next;
  struct VHostConf* p = vhostList;
  for ( ; p; p = next) {
    next = p->next;
    MyFree(p->vhost);
    MyFree(p->pass);
 ...
 * @return #vhostList
 */
const struct VHostConf* conf_get_vhost_list(void)
{
  return vhostList;
}
