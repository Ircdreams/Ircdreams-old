 ...
 *
 * $Id: whocmds.c,v 1.5 2005/01/24 01:19:24 bugs Exp $
 */
#include "../config.h"
            ...
            int fields, char* qrt)
{
  ...
  if (IsHiding(acptr) && !IsAnOper(sptr)) return;
  ...
  {
    char *p2 = !IsAnOper(sptr) ? cli_user(acptr)->crypt : cli_user(acptr)->host;
    *(p1++) = ' ';
    while ((*p2) && (*(p1++) = *(p2++)));
  }
