...
int ms_svshost(struct Client* cptr, struct Client* sptr, int parc, char* parv[])
{
  ...
  if (legalhost == 0)   
    return send_reply(sptr, ERR_SVSCOM, "SVSHOST", "caractères invalides dans le hostname");
...
int ms_svsnick(struct Client* cptr, struct Client* sptr, int parc, char* parv[])
{
  ...
  if (!acptr) return protocol_violation(sptr,"SVSNICK: Le pseudo donné n'est pas un user.");
