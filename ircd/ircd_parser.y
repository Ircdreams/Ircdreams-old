 ...
 *  USA.
 * $Id: ircd_parser.y,v 1.7 2005/12/24 14:50:40 progs Exp $
 */
%{
...
#define MAX_STRINGS 80 /* Maximum number of feature params. */
  ...
  extern struct VHostConf*  vhostList;
...
%token TPRIV_JOINX TPRIV_MODE_LCHAN TPRIV_PROTECT TPRIV_CHAN_LIMIT
%token TPRIV_LOCAL_KILL TPRIV_REHASH TPRIV_RESTART TPRIV_DIE TPRIV_LIST_CHAN
...
%token TPRIV_FORCE_OPMODE TPRIV_FORCE_LOCAL_OPMODE TPRIV_APASS_OPMODE TPRIV_OPERFREEMODE
/* and some types... */
%type <num> sizespec
...
block: adminblock | generalblock | classblock | connectblock |
       ...
       killblock | cruleblock | motdblock | vhostblock | featuresblock | quarantineblock |
...
privtype: TPRIV_MODE_LCHAN { $$ = PRIV_MODE_LCHAN; } |
          ...
          TPRIV_OPERFREEMODE { $$ = PRIV_OPERFREEMODE; } |
          TPRIV_LIST_CHAN { $$ = PRIV_LIST_CHAN; } |
          TPRIV_PROTECT { $$ = PRIV_PROTECT; } |
          TPRIV_JOINX { $$ = PRIV_JOINX; } ;
...
vhostblock: SETHOST
{
  vconf = (struct VHostConf*) MyCalloc(1, sizeof(*vconf));
} '{' vhostitems '}' ';'
{
  if (vconf->vhost) {
    vconf->next = vhostList;
    vhostList = vconf;
  }
  else
  {
    MyFree(vconf->vhost);
    MyFree(vconf->realhost);
    MyFree(vconf->pass);
    MyFree(vconf);
    parse_error("You have to define a vhost !");
  }
  vconf = NULL;
};
vhostitems: vhostitem vhostitems | vhostitem;
vhostitem: vhostvhost | vhostrealhost | vhostpass | vhostprefixed;
vhostvhost: VHOST '=' QSTRING ';'
{
/*  char *h;
  MyFree(vconf->vhost);
  MyFree(vconf->vuser);
  ...
  {
    *h++ = '\0';
    DupString(vconf->vuser, h);
    vconf->vhost = $3;
  }*/
  MyFree(vconf->vhost);
  vconf->vhost = $3;
};
vhostpass: PASS '=' QSTRING ';'
{
  MyFree(vconf->pass);
  vconf->pass = $3;
};
vhostrealhost: REALHOST '=' QSTRING ';'
{
  MyFree(vconf->realhost);
  vconf->realhost = $3;
};
vhostprefixed: PREFIXED '=' yesorno ';'
{
  if ($3 == 1)
    vconf->flags |= VHOST_FLAGS_PREFIX;
  else
    vconf->flags &= ~VHOST_FLAGS_PREFIX;
};
