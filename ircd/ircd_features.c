 ...
 *
 * $Id: ircd_features.c,v 1.50 2005/12/08 17:10:48 bugs Exp $
 */
#include "../config.h"
...
#define F_S(type, flags, v_str, notify)					      \
  ...
  F_S(HIDDEN_HOST, FEAT_CASE, "users.ircdreams.org", 0),
  ...
  F_B(CRYPT_OPER_PASSWORD, FEAT_MYOPER | FEAT_READ, 0, 0),
  ...
  F_S(HIS_SERVERNAME, 0, "*.ircdreams.org", 0),
  F_S(HIS_SERVERINFO, 0, "Réseau des coders de ircdreams.org", 0),
  F_S(HIS_URLSERVERS, 0, "http://www.ircdreams.org", 0),

  /* Misc. random stuff */
  F_S(NETWORK, 0, "IrcDreams.org", 0),
  F_S(URL_CLIENTS, 0, "ftp://ftp.undernet.org/pub/irc/clients", 0),
  F_S(KLINE_MAIL, 0, "Klines@IrcDreams.org", 0),
  F_B(HIS_SERVERMODE, 0, 0, 0),
  F_B(OPCLEARMODE, 0, 1, 0),
  F_B(SETHOST_AUTO, 0, 1, 0),
  
  /* IrcDreams Features */
  
  F_B(AUTOJOIN_OPER, 0, 1, 0),
  F_B(AUTOJOIN_OPER_NOTICE, 0, 1, 0),
  F_S(AUTOJOIN_OPER_NOTICE_VALUE, 0, "*** Notice -- Vous allez maintenant rejoindre automatiquement #Q", 0),
  F_S(AUTOJOIN_OPER_CHANNEL, 0, "#Q", 0),
  F_S(OMPATH, FEAT_CASE | FEAT_MYOPER | FEAT_READ, "ircd.opermotd", 0),
  F_B(AUTOJOIN_USER, 0, 1, 0),
  F_B(AUTOJOIN_USER_NOTICE, 0, 1, 0),
  F_S(AUTOJOIN_USER_NOTICE_VALUE, 0, "*** Notice -- Vous allez maintenant rejoindre automatiquement #IrcDreams", 0),
  F_S(AUTOJOIN_USER_CHANNEL, 0, "#IrcDreams", 0),
  F_S(EPATH, FEAT_CASE | FEAT_MYOPER, "ircd.rules", 0),
  F_B(RULES, 0, 0, 0),
  F_I(PROTECTHOST, 0, 2, 0),
  F_B(HIS_STATS_b, 0, 1, 0),
  F_B(AUTOINVISIBLE, 0, 1, 0),
  F_B(TOO_MANY_FROM_IP, 0, 1, 0),
  F_B(LOG_GESTION_MAIL, 0, 0, 0),
  F_S(PROG_MAIL, 0, "/usr/sbin/sendmail -t", 0),
  F_S(GESTION_MAIL, 0, "gestion@IrcDreams.org", 0),
  F_B(ALERTE_OPER, 0, 0, 0),
  F_B(ALERTE_SETHOST, 0, 0, 0),
  F_B(ALERTE_GLINE, 0, 0, 0),
  F_B(ALERTE_NETSPLIT, 0, 0, 0),
  ...
  F_B(HIS_STATS_S, 0, 1, 0),
  F_B(OPERMOTD, 0, 0, 0),
  F_B(WALL_CONNEXIT_NOTICES, 0, 0, 0),
...
feature_desc(struct Client* from, const char *feature)
{
  ...
  Debug((DEBUG_ERROR, "Caractéristique inconnue \"%s\"", feature));
  if (from) /* report an error */
    send_reply(from, ERR_NOFEATURE, feature);
  else
    log_write(LS_CONFIG, L_ERROR, 0, "Caractéristique inconnue \"%s\"", feature);
...
feature_set(struct Client* from, const char* const* fields, int count)
{
  ...
  if (from && (!HasPriv(from, PRIV_SET) || !CanSetVars(from)))
  ...
  if (count < 1) {
	...
	change++;
      ...
      if(from) send_reply(from, SND_EXPLICIT | RPL_FEATURE,
		 ":La valeur de %s: %d", feat->type, feat->v_int);
      break;
	...
	if (from)
	  ...
	  log_write(LS_CONFIG, L_ERROR, 0, "Mauvaise valeur \"NULL\" pour la feature %s",
		    feat->type);
	  return 0;
	}
      }
	...
	feat->flags |= FEAT_MARK;
	  ...
	  send_reply(from, SND_EXPLICIT | RPL_FEATURE,
		   ":La valeur de %s: %s", feat->type, feat->v_str);
        else
	  send_reply(from, SND_EXPLICIT | RPL_FEATURE,
		   ":La valeur de %s n'a pas été configurée", feat->type);
      }
      break;
    }
    ...
    if (change && from) {
      ...
      if(!IsServer(from) && !IsMe(from))
        send_reply(from, SND_EXPLICIT | RPL_FEATURE, ":La valeur de %s a bien été changé",
                   feat->type);

       switch (feat->flags & FEAT_MASK) {
          case FEAT_NONE:
            sendto_opmask_butone(from, SNO_OLDSNO, "%C change la valeur de %s",
                                 from, feat->type);
            break;
          case FEAT_INT:
            sendto_opmask_butone(from, SNO_OLDSNO, "%C change la valeur de %s en %d",
                                 from, feat->type, feat->v_int);
            break;
          case FEAT_BOOL:
            sendto_opmask_butone(from, SNO_OLDSNO, "%C change la valeur de %s en %s", from,
                                 feat->type, (feat->v_int ? "TRUE" : "FALSE"));
            break;
          case FEAT_STR:
            if(feat->v_str)
              sendto_opmask_butone(from, SNO_OLDSNO, "%C change la valeur de %s en %s",
                                   from, feat->type, feat->v_str);
            else
              sendto_opmask_butone(from, SNO_OLDSNO, "%C enlève %s",
                                   from, feat->type);
            break;
        }
      }
...
feature_reset(struct Client* from, const char* const* fields, int count)
{
  ...
  if (!HasPriv(from, PRIV_SET) || !CanSetVars(from))
    return send_reply(from, ERR_NOPRIVILEGES);
  ...
  if (count < 1) /* check arguments */
    ...
    if(from && !IsServer(from) && !IsMe(from))
      send_reply(from, SND_EXPLICIT | RPL_FEATURE, ":La Valeur de %s a bien été remise à la valeur par "
                 "défaut", feat->type);
      sendto_opmask_butone(from, SNO_OLDSNO, "%C remet la valeur par défaut à %s",
                           from, feat->type);    
  }
