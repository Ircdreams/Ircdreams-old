 ...
 * @version $Id: ircd_snprintf.c,v 1.2 2005/10/26 22:56:24 progs Exp $
 */
#include "config.h"
...
/*      } else if ((fld_s.flags & CONV_MASK) == CONV_FLOAT) { */
    ...
    } else if (((fld_s.flags & CONV_MASK) == CONV_CLIENT) || ((fld_s.flags & CONV_MASK) == CONV_REAL)) {
	...
	if (!IsServer(cptr) && !IsMe(cptr) && fld_s.flags & FLAG_ALT) {
	  ...
	  if ((fld_s.flags & CONV_MASK) == CONV_REAL) {
	    str2 = cli_user(cptr)->username; /* TODO: à changer pour realusername si on le créé */
