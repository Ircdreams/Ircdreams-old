...
/** @file
 * @brief CHECK command
 * @version $Id: m_check.c,v 1.4 2005/10/25 14:24:54 progs Exp $
 */
...
void checkClient(struct Client *sptr, struct Client *acptr) {
  ...
  ircd_snprintf(0, outbuf, sizeof(outbuf), "  User/Hostmask:: %s@%s", acptr->cli_user->username,
  						acptr->cli_user->host);
  send_reply(sptr, RPL_DATASTR, outbuf);

  if (strcmp(cli_user(acptr)->realhost, cli_user(acptr)->host)) {
  ...
  if (acptr->cli_user->joined == 0)
    ...
    *chntext = '\0';
    ...
    for (lp = acptr->cli_user->channel; lp; lp = lp->next_channel) {
      ...
      if (IsDeaf(acptr))
        *(chntext + len++) = '-';
      if (IsOper(acptr) && !ShowChannel(acptr, chptr))
        *(chntext + len++) = '%';
  ...
  if (MyUser(acptr)) {
    send_reply(sptr, RPL_DATASTR, " ");
    ircd_snprintf(0, outbuf, sizeof(outbuf), "          Ports:: %d (server)",
           cli_listener(acptr)->addr.port);
    send_reply(sptr, RPL_DATASTR, outbuf);
    if (feature_bool(FEAT_EXTENDED_CHECKCMD)) {
      /* Note: sendq = receiveq for a client (it makes sense really) */
      ircd_snprintf(0, outbuf, sizeof(outbuf), "      Data sent:: %u bytes (%u protocol messages)",
            cli_receiveB(acptr), cli_receiveM(acptr));
      send_reply(sptr, RPL_DATASTR, outbuf);
      ircd_snprintf(0, outbuf, sizeof(outbuf), "  Data received:: %u bytes (%u protocol messages)",
            cli_sendB(acptr), cli_sendM(acptr));
      send_reply(sptr, RPL_DATASTR, outbuf);
...
void checkServer(struct Client *sptr, struct Client *acptr) {
  ...
  ircd_snprintf(0, outbuf, sizeof(outbuf), "          Users:: %d / %d", (acptr == &me ?
  					UserStats.local_clients : cli_serv(acptr)->clients),
...
signed int checkHostmask(struct Client *sptr, char *hoststr, int flags) {
  ...
  char *p = NULL;
/* TODO: CORRIGER CE PUTAIN DE BORDEL DE MERDE DE IRC_IN_ADDR */
#define NO_CIDDR_CHECK_MASKS
#ifndef NO_CIDDR_CHECK_MASKS
  ...
  for (acptr = GlobalClientList; acptr; acptr = cli_next(acptr)) {
    ...
    if (flags & CHECK_CIDRMASK) {
      if (((cli_ip(acptr).s_addr & NETMASK(cidr_check_bits)) == cidr_check.s_addr) && !match(nickm, acptr->cli_name)
            && (!match(userm, acptr->cli_user->username)))
        found = 1;
    }
    else
#endif
