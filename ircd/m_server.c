 ...
 *
 * $Id: m_server.c,v 1.5 2005/01/24 00:54:39 bugs Exp $
 */
...
int mr_server(struct Client* cptr, struct Client* sptr, int parc, char* parv[])
{
  ...
  if (strlen(parv[5]) != 3 || (parv[5][0] != 'P' && parv[5][0] != 'J'))
    return exit_client_msg(cptr, sptr, &me, "Erreur de protocole (%s)", parv[5]);
  ...
  if (IsUnknown(cptr) || IsHandshake(cptr))
  {
    ...
    if (0 != conf_eval_crule(host, CRULE_ALL)) {
      ServerStats->is_ref++;
      sendto_opmask_butone(0, SNO_OLDSNO, "Connexion refusée depuis %s.", cli_name(cptr));
      return exit_client(cptr, cptr, &me, "Disallowed by connection rule");
    }

    if (conf_check_server(cptr)) {
      ++ServerStats->is_ref;
      sendto_opmask_butone(0, SNO_OLDSNO, "Connexion non autorisée reçue depuis %s.", cli_name(cptr));
      ...
      return exit_client(cptr, cptr, &me, "Aucune C:line");
    }
         ...
         (parc > 7 && (acptr = FindNServer(parv[6]))))
  {
    ...
    if (0 != ircd_strcmp(cli_name(acptr), host)) {
      sendcmdto_serv_butone(&me, CMD_WALLOPS, cptr,
			    ":Collision de Numerique entre serveur: %s != %s",
			    cli_name(acptr), host);
      return exit_client_msg(cptr, cptr, &me,
          "Collision de Numerique entre %s et %s."
          " Le numerique du serveur est-il correct ?", host, cli_name(acptr));
    }
    /*
...
int ms_server(struct Client* cptr, struct Client* sptr, int parc, char* parv[])
{
         ...
         (parc > 7 && (acptr = FindNServer(parv[6]))))
  {
    ...
    {
      sendcmdto_serv_butone(&me, CMD_WALLOPS, cptr,
			    ":Collision de Numerique entre serveur: %s != %s", cli_name(acptr),
			    host);
      return exit_client_msg(cptr, cptr, &me,
	"Collision de Numerique entre %s et %s."
        " Le numerique du serveur est-il correct ?", host, cli_name(acptr));
    ...
    {
      if (!active_lh_line && exit_client(cptr, acptr, &me,
          "Deja connecté depuis un autre link") == CPTR_KILLED)
        return CPTR_KILLED;
      /*
       * We can have only ONE 'IsConnecting', 'IsHandshake' or
       * 'IsServer', because new 'IsConnecting's are refused to
