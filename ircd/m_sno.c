...
/** @file
 * @brief SNO command
 * @version $Id: m_sno.c,v 1.2 2005/10/23 13:58:07 progs Exp $
 */
...
/*
 * ms_sno - envoi d'une snotice à *TOUS* les users ayant le snotice mask correspondant
 *
 * parv[1] = mask
 * parv[2] = msg
 */
int ms_sno(struct Client* cptr, struct Client* sptr, int parc, char* parv[])
{
  ...
  sendto_allops_butserv(cptr, sptr, mask, "%s", message);
  return 0;
}
