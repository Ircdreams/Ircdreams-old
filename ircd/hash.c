 ...
 * @version $Id: hash.c,v 1.3 2005/10/15 09:57:01 progs Exp $
 *
  ...
  for (args = cli_listing(cptr); args->bucket < HASHSIZE; args->bucket++)
  {
    ...
    for (chptr = channelTable[args->bucket]; chptr; chptr = chptr->hnext)
    {
      if (chptr->users > args->min_users
          ...
          && ((args->flags & LISTARG_SHOWSECRET) /* <- avec /list s, permet l'affichage */
              || ShowChannel(cptr, chptr)))      /* de TOUS les chans par un ircop      */
      {
	if (IsAnOper(cptr))
	{
	  ...
	  send_reply(cptr, RPL_LIST, chptr->chname, chptr->users, topic);
	}
	else
